...
var isView = readLine()!.split(separator: " ").map{Int($0)!}
isView[n-1] = 0
var graph = [[Node]](repeating: [], count: n)
for _ in 1...m {
    ...
    graph[relationship[1]].append(Node(vertex: relationship[0], distance: relationship[2]))
}

let initialValue = Int.max
var distances = [Int](repeating: initialValue, count: n)
distances[0] = 0
var queue = Heap<Node> {
    $0.distance < $1.distance
}
queue.enqueue(Node(vertex: 0, distance: 0))
while let node = queue.dequeue() {
    ...
    for next in graph[node.vertex] {
        ...
        if isView[next.vertex] == 0 && distances[next.vertex] > dist {
            distances[next.vertex] = dist
            queue.enqueue(Node(vertex: next.vertex, distance: dist))
        }
    }
}
distances[n-1] == Int.max ? print(-1) : print(distances[n-1])